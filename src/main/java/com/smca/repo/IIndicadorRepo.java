...
public interface IIndicadorRepo extends JpaRepository<Indicador, Integer>{
	...
	@Query(value="select ind.* from indicador ind , (select  idestacion, max(fecharegistro) as fecharegistro from indicador where idestacion=?1 group by idestacion) tmp where ind.idestacion=tmp.idestacion and ind.fecharegistro=tmp.fecharegistro order by ind.fecharegistro",nativeQuery=true)
